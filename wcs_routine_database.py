...
def load_routine_data():
        return (pl.concat([pl.scan_csv('routine_videos.csv'), 
                           pl.scan_csv('Fabio_Routine_Archive.csv', infer_schema_length=0).rename({'Link':'url'})
                           ], 
                ...
                .with_columns(
                            ...
                            #                     .list.unique()
                            #                     .list.drop_nulls(),
                                extracted_year = pl.concat_list(pl.all(pl.String).str.extract_all(pattern_yyyy),
                                                                pl.all(pl.String).str.extract_all(pattern_apos_yy),
                                                                )
